...
module.exports = async function createServer() {
  ...
  app.use(async (ctx, next) => {
    const logger = getLogger();
    logger.debug(`token: ${ctx.headers.authorization}`); // 👈 1
    logger.debug(`current user: ${JSON.stringify(ctx.state.user)}`); // 👈 2
    logger.debug(`error in token: ${ctx.state.jwtOriginalError}`); // 👈 3
    await next();
  });
